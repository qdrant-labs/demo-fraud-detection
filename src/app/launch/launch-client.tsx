...
// Client half of the launcher: the persona card, three attack cards, the live
// launch status (per-event progress + running highest score streamed from
// /api/attack as NDJSON), and a compact embedded wall strip so the demo makes
// sense from a single browser tab when no projected wall is open.
  ...
  return (
    <main className="min-h-screen bg-[#07090d] px-5 py-8 text-slate-100">
      <div className="mx-auto flex w-full max-w-xl flex-col gap-6">
        ...
        {/* Compact embedded wall, opened once a launch begins */}
        {status !== "idle" ? <WallStrip /> : null}
      </div>
    </main>
  );
}
