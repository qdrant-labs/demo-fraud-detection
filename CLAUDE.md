...
- **Persisted fraud events crowd a persona's baseline out of the context window, and then nothing alerts.** The context scroll (`score.ts` step 1) takes the tenant's newest `CONTEXT_LIMIT` (30) points by `ts`. `processBucket` persists every injected motif (`persist: tx.motif !== "none"`), so a booth session piles fraud events onto the 200 live personas, and the janitor only removes them after 24 h. Once 30/30 of a persona's newest points are prior fraud injections, a fresh impossible-travel charge looks ordinary next to them: `d_local` inflates, the ratio falls under 2.0, and the wall shows nothing. Measured 2026-07-28: **all 20 sampled personas were at 30/30**, manual attacks on `t0011` scored 1.18-1.27 and never alerted, and the same launches after clearing scored 5.49 (geo_hop) and 3.09-3.23 (card_testing). This is the 2026-07-12 49%-alert incident in reverse - same crowding, opposite sign - so treat "manual attack does nothing" as this first.
  - **Remedy (proven, needs Dylan's approval each time):** `POST /collections/fraud_demo/points/delete?wait=true {"filter":{"must_not":[{"is_empty":{"key":"score"}}]}}`. Baselines carry no `score`, so they all survive; verify the count lands on exactly **10,971,318**, the sum of `makeProfile().txCount` over 20,000 cardholders. Ran 2026-07-28, removed 13,124 scored points, landed exactly on that number.
  - **Durable fix, drafted and NOT shipped:** give the scroll the guard the kNN already has. The kNN protects itself from a forming burst with `NEIGHBOR_EXCLUDE_MS`; the scroll has no equivalent, which is the whole asymmetry. Add a `should` clause so a scored point only enters the window when it is inside a burst window, while baselines always qualify - `must: [tenant_id, ts < event.ts]`, `should: [{is_empty: {key: "score"}}, {key: "ts", range: {gte: event.ts - 20min}}]` (Qdrant reads this as must AND (should OR should)). Bursts still form, evidence retention is untouched, and accumulation stops mattering. It changes scoring semantics, so it needs the motif-detection and cold-start evals before it ships, which needs `docker start qdrant-bench` back.
  - Janitor sweep **frequency is not the lever** here, so do not reach for it: the binding constraint is `JANITOR_MAX_AGE_MS` (24 h), not how often the sweep runs. Lowering the age cutoff would bound accumulation but costs alert-evidence lifetime, which is why the `should` clause is the better trade.
